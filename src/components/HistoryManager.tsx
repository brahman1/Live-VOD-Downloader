  ...
  if (tasks.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-slate-500 animate-in fade-in bg-white border border-slate-200 rounded-3xl shadow-sm">
        ...
        <h3 className="text-xl font-extrabold text-slate-800">{t('HISTORY_EMPTY')}</h3>
        <p className="text-sm mt-2 font-medium text-slate-500">{t('HISTORY_EMPTY_DESC')}</p>
      </div>
    );
  }
    ...
    <div className="space-y-4 animate-in fade-in flex-1 overflow-y-auto pr-2 pb-20">
      <div className="flex items-center justify-between mb-6">
         <h2 className="text-xl font-black text-slate-900 flex items-center gap-2">
           <History className="w-5 h-5 text-slate-500" /> {t('TAB_HISTORY')}
         </h2>
         <button onClick={onClearAll} className="px-4 py-2 bg-slate-100 text-slate-600 hover:text-red-600 rounded-xl text-xs font-bold transition-colors border border-slate-300 shadow-sm">
           {t('CLEAR_HISTORY')}
         </button>
      </div>

      {tasks.slice().reverse().map((task) => (
        <div key={task.id} className="bg-white border border-slate-200 p-4 rounded-2xl flex flex-col gap-3 transition-all duration-300 shadow-sm hover:shadow-md">
          ...
          <div className="flex gap-4 items-start">
            ...
            <button
                ...
                className="p-2 text-slate-400 hover:bg-red-50 hover:text-red-600 rounded-lg transition-all duration-300 group"
                title={t('REMOVE_BTN')}
              >
          ...
          <div className="mt-2">
            {task.status === 'error' ? (() => {
              ...
              return (
               ...
               <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 rounded-lg text-xs font-mono text-slate-600 border border-slate-200 break-all shadow-sm">
